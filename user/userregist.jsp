		...
		function validate(field) {
			...
			if (!phoneNumReg.test(username)) {
				...
				if (trim(field.value).length != 0) {
					...
					var url = "uservalidate.jsp?username=" + trim(field.value);
						...
						if (xmlHttp.readyState == 4) {
							...
							if (xmlHttp.status == 200) {
								if (trim(xmlHttp.responseText) != "") {
									...
									document.getElementById("spanusername").innerHTML = "<font color='red'>"
				...
				<div class="PnBox beee">
					<div>
						<span class="fa fa-barcode ur_span1"></span>
						 <input name="passcode" id="passcode" placeholder="填写验证码"class="Pn w100 f18"
							data-validate="required:请填写验证码" onblur="checkpasscode()"/> 
							<input value="获取验证码"
							type="button" id="btnSendCode" name="btnSendCode" onclick="sendMessage()" class="sendtoo-grenn f14" >
					</div>
				</div>
				<div id="spancode"></div>	
			</p>
